...
const main = {
  ...
  gchunk: require("../database/maps/gdrive_chunk.json"),
...
//Gchunk Property
updateGchunk()
setInterval(updateGchunk, 3600000);

async function updateGchunk() {
  try {
    const response = await axios.get('https://cdn.bc5f39.workers.dev/maps/gdrive_chunk.json');
    const data = response.data;
    main.gchunk = data;
    shareLog('GDRIVE_CHUNK', 'Hourly Chunk Fetched');
    fs.writeFileSync(`${__dirname}/../database/maps/gdrive_chunk.json`, JSON.stringify(data));
  } catch (error) {
    shareLog('GDRIVE_CHUNK', `Error: ${error.message}`);
  }
}
