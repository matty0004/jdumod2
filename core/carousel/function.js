...
function filterSongsByJDVersion(songdbs, filter) {
  ...
  filteredSongdbs.sort((a, b) => {
    const titleA = songdb[a].title.toLowerCase();
    const titleB = songdb[b].title.toLowerCase();
    return titleA.localeCompare(titleB);
  });
  return filteredSongdbs;
}
...
function addJDVersion(songdbs) {
  addCategories(generateCategories("Just Dance 2024", filterSongsByJDVersion(songdbs, 2024)))
  addCategories(generateCategories("Just Dance 2023", filterSongsByJDVersion(songdbs, 2023)))
